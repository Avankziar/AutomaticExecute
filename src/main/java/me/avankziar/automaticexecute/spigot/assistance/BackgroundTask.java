...
import main.java.me.avankziar.automaticexecute.spigot.object.Title;
...
public class BackgroundTask
{
	...
	public void initList()
	{
		...
		YamlConfiguration auto = yh.getA();
		String sepb = yh.get().getString("Seperator.BetweenFunction", "~");
		for(String path : yh.getA().getKeys(false))
		{
			AutomaticExecute.log.info("Loading Message: "+path);
			if(auto.getString(path+".Rythmus") == null) continue;
			AutoMessage.Rythmus rythmus = AutoMessage.Rythmus.valueOf(auto.getString(path+".Rythmus", "ONCE"));
	...
	public void autoSendMessage(LocalDateTime now)
	{
		...
		new BukkitRunnable()
		{
			...
			public void run()
			{
				...
				{
					if(am.getRythmus() == AutoMessage.Rythmus.ONCE)
					{
						doONCE(am, day, month, year, hour, min, sec);
					} else if(am.getRythmus() == AutoMessage.Rythmus.ONCE_A_DAY)
					{
						doONCE_A_DAY(am, hour, min, sec);
					} else if(am.getRythmus() == AutoMessage.Rythmus.ON_TIMES)
					{
						doON_TIMES(am, hour, min, sec);
					} else if(am.getRythmus() == AutoMessage.Rythmus.MULTIPLE_ON_THE_DAY)
					{
						doMULTIPLE_ON_THE_DAY(am, day, month, year, hour, min, sec);
					} else if(am.getRythmus() == AutoMessage.Rythmus.INTERVAL)
					{
